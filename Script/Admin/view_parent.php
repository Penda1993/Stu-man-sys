        ...
        <div class="container-fluid" style="height:500px;">
            <div class="row">
                ...
                <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
                   <center>
		<div  id="container">
		<br><center><p style="font-size:30px;color:red;font-weight:bold;"><span style="color:black;">ABOUT</span> ALL PARENTS</p></center>
<br>
			<div class="container" >
				<table border="1" class="table" id="table">
					...
					<tr id="tr1">
						<th id="th1">Name</th>
						<th id="th1">Photo</th>
						<th id="th1">Child's Name</th>
						<th id="th1">Child's Student ID</th>
...
<?php
	include '../db.php';
	$sql=mysqli_query($conn,"SELECT * FROM parent_login");
	$s=0;
	while($data=mysqli_fetch_assoc($sql)){
		...
		echo '
			...
			<tr id="tr2">
						<th>'.$s.'</th>
						<th><img src="../parent/img/'.$data['img'].'" class="rounded float-left" style="height:60px;width:70px;"></th>
						<th>'.$data['f_name'].' '.$data['l_name'].'</th>
						<th>'.$data['child_name'].' '.$data['ch_std_id'].'</th>
						<th>'.$data['contact'].'</th>
						<th>'.$data['email'].'</th>
						<th>'.$data['address'].','.$data['city'].','.$data['province'].'</th>
					</tr>
