<?php
	...
	{
		$b=mysqli_query($conn,"insert into parent_login(`f_name`,`l_name`,`gender`,`email`,`address`,`province`,`city`,`zipcode`,`contact`,`password`,`child_name`,`ch_std_id`,`img`) values('".$_POST['f_name']."','".$_POST['l_name']."','".$_POST['gender']."','".$_POST['email']."','".$_POST['address']."','".$_POST['province']."','".$_POST['city']."','".$_POST['zipcode']."','".$_POST['contact']."','".$_POST['password']."','".$_POST['c_fname']."','".$_POST['ch_std_id']."','".$_FILES['image']['name']."')");
		$name=$_FILES['image']['name'];
        $temp=$_FILES['image']['tmp_name'];
        $folder="../student/img/";
        $d=move_uploaded_file($temp,$folder.$name);
		$data=mysqli_fetch_assoc(mysqli_query($conn,"select * from parent_login where id=".mysqli_insert_id($conn).""));
		if($b)
		{
			echo'
			<script>
...
<html>
	...
	<?php include 'header.php';?>
		...
		<center><p style="font-size:28px;color:;font-weight:bold;"><span style="color:red;">ADD PARENT</span> FORM</p></center>
				...
				<div class="col-md-8 p-5">
					...
					<!--row start-->
						...
						<div class="row"> 
							<div class="form-group col-md-4">  <!--comumn2-->
								<input  name="province" type="text" placeholder="province" class="form-control"  style="height:40px; border-radius:7px;font-size:16px; box-shadow: 2px 2px 15px #888888;" required>
							...
							<div class="form-group col-md-4">  <!--comumn2-->
							<input name="zipcode" type="number" class="form-control" Placeholder="zipcode" style="height:40px; border-radius:7px;font-size:16px; box-shadow: 2px 2px 15px #888888;" required>
							</div>
						</div>
						<!--row end-->
						...
						<div class="form-group">
							<div class="input-group mb-3" style=" border-radius:7px;font-size:16px; box-shadow: 2px 2px 15px #888888; height:40px;">
								  <div class="input-group-prepend">
									<span class="input-group-text" style="color:blue;font-weight:bold;">+86</span>
								  </div>
								  <input name="contact" type="number" class="form-control" placeholder="contact Number" style="height:40px;">
								</div>							
						</div>
						<!--contact no. end-->
					...
					<div class="row">
								<!--column 1-->
									<div class="form-group has-search">
									<span class="fa fa-user-circle form-control-feedback"></span>
									<input name="c_fname" type="text" class="form-control" placeholder="Child's Name" style="border-radius:10px;height:40px;font-size:16px; box-shadow: 2px 2px 15px #888888;" required>
									</div>
							
							 	<!--comumn2-->
								<div class="form-group has-search">
									<span class="fa fa-user-circle form-control-feedback"></span>
									<input name="ch_std_id" type="text" class="form-control" placeholder="Student ID" style="border-radius:10px;height:40px;font-size:16px; box-shadow: 2px 2px 15px #888888;" required>
								</div>
						...
						<span class="fa fa-institution form-control-feedback"></span>
							<select name="course" class="form-control"  style="height:40px; border-radius:7px;font-size:16px; box-shadow: 2px 2px 15px #888888;" required>
								<option selected>Choose Child's Course</option>
								<?php
