<?php 
	...
	{
		$users_check=mysqli_query($conn,"select * from parent_login where email='".$_POST['email']."' and password='".$_POST['password']."'");
		$users_count=mysqli_num_rows($users_check);
		if($users_count==1)
		{
			$users=mysqli_fetch_assoc(mysqli_query($conn,"select * from parent_login where email='".$_POST['email']."' and password='".$_POST['password']."'"));
			session_start();
